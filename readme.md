...
├── deploy_iraypleos/               # IRAYPLEOS/CentOS 7 离线部署
│   ├── deploy_iraypleos.sh         # 主部署脚本 (幂等)
│   ├── prepare_portable_python.sh  # 便携 Python 制作工具
│   ├── README.md
│   ├── platform/
│   │   └── centos7/
│   │       ├── setup.sh            # CentOS 7 专属 Python 安装
│   │       └── rpms/               # 离线 Python 包/SRC 放置目录
│   └── vendor_packages3.9/         # 离线 wheel 包
│       ├── requirements_asap_fixed.txt
│       └── *.whl
├── dev/
│   └── start_all.sh                # 一键启动模拟器 + ASAP
├── test/                           # 测试脚本 (gitignore)
...
| `/api/asap/config/rcs` | GET | 获取 RCS 配置 | ASAP |
| `/api/asap/config/rcs` | POST | 更新 RCS 配置 (运行时生效) | ASAP |
| `/api/asap/upgrade/version` | GET | 获取当前版本号 | ASAP |
| `/api/asap/upgrade/records` | GET | 获取升级记录 | ASAP |
| `/api/asap/upgrade/upload` | POST | 上传 ZIP 升级包 | ASAP |
| `/api/asap/upgrade/rollback/{name}` | POST | 回滚到指定备份 | ASAP |
...
| `angel` | `base_url` | `http://localhost:5112` | 风淋门 HTTP 基础地址（调试时指向模拟器） |
...
| `rcs` | `change_status_url` | - | RCS 状态上报地址（WebUI 可直接编辑） |
...
| 功能 | 说明 |
|------|------|
| **门状态面板** | 实时显示外门/内门状态（关闭/打开中/已打开/故障），支持手动开/关/故障注入 |
| **区域管控面板** | 查看区域占用状态，支持强制占用/释放/始终繁忙模式 |
| **模拟配置** | 调整开门/关门过渡延时 |
| **快速测试** | 一键模拟风淋流程、区域冲突、门故障 |
| **请求日志** | 记录所有 API 请求，点击 `📄` 按钮可查看完整请求/响应报文（格式化 JSON） |
...
ASAP WebUI 首页底部实时显示配置地址（自动填充当前主机和端口），状态上报 URL 支持直接编辑保存。
...
| `change_status_url` | `http://<RCS_HOST>/changeDoorStatus` | 状态上报（WebUI 中可编辑） |
...
```bash
# 打包需要更新的文件（从项目根目录执行）
zip -r upgrade_v1.1.0.zip asap_adapter/ requirements.txt

# 可选放入 version.json（版本说明），升级包根目录：
echo '{"title":"v1.1.0","changes":["修复xxx","新增xxx"],"from_version":"1.0.0"}' > version.json
# 如果 from_version 与当前服务器版本不一致，升级将被拒绝
```

### WebUI 操作

打开 `http://localhost:5012/upgrade`：

1. **上传区**：拖拽或点击选择 `.zip` 文件
2. **升级说明**：可选输入备注（或在 `version.json` 中写 `changes`）
3. **开始升级**：自动备份当前代码 → 解压覆盖 → 记录日志 → 重启服务
4. **升级记录**：表格展示历史升级（时间、版本变化、升级说明、文件数）
5. **回滚**：每条成功升级记录旁有「回滚」按钮，一键恢复到该版本

### 排除清单

以下文件/目录在升级中自动**保留**，不受影响：
`config/env.toml`, `config/old/`, `venv/`, `logs/`, `backup/`, `dev/`, `test/`, `.git/`, `.gitignore`, `skill.md`, `README.md`, `deploy_iraypleos/`, `__pycache__/`, `*.pyc`
...
- 操作系统：CentOS 7 / openEuler / Debian
- Python：自动检测安装（≥ 3.9）
- 网络：完全离线
...
脚本自动执行：
1. **OS 检测** — 识别 CentOS 7 / openEuler / Ubuntu
2. **Python 检测** — 按优先级查找：`python3` → SCL `/opt/rh/rh-python39/` → `/usr/local/python3/` → `python`
3. **Python 安装** — 未找到时自动尝试：便携包 → yum → dnf → apt-get
4. **虚拟环境创建** — 使用检测到的 Python 创建 venv
5. **离线依赖安装** — 从 `vendor_packages3.9/` 批量安装 wheel 包
6. **Supervisor 配置** — 自动创建 `/main/server/supervisor/asap_adapter.conf`
7. **服务启动** — supervisorctl → 直接 `nohup`

### 便携 Python

如果目标机器无 Python 且无包管理器，可制作便携包：

```bash
# 在开发机上运行（需要 Python 3.9+）
bash deploy_iraypleos/prepare_portable_python.sh

# 生成的便携包在 vendor_packages3.9/python3.9/
# 将整个项目 scp 到目标机器后运行部署脚本即可
```
...
升级包上传后先备份当前代码再覆盖，备份保留最近 10 个版本，支持一键回滚。`config/env.toml` 等敏感文件自动排除，不受升级影响。
