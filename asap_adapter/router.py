...
def create_router(app: FastAPI) -> APIRouter:
    ...
    async def update_rcs_config(request: Request, cfg: RcsConfigUpdate):
        ...
        logger.info("RCS门编码映射已更新: %s", cfg.door_code_mapping)
        return {"status": "ok", "door_code_mapping": config.rcs.door_code_mapping}
    ...
    @router.get("/api/asap/config/export")
    async def export_config():
        """导出当前 data/config.toml"""
        from .config import export_config
        content = export_config()
        return PlainTextResponse(content, media_type="text/plain",
                                  headers={"Content-Disposition": "attachment; filename=config.toml"})

    @router.post("/api/asap/config/import")
    async def import_config(request: Request):
        """导入配置（上传文件或粘贴文本）"""
        from .config import import_config, _load_unified
    ...
    @router.get("/api/asap/config/versions")
    async def list_versions():
        """列出配置版本历史"""
        from .config import list_config_versions
        versions = list_config_versions()
        return {"success": True, "versions": versions, "current": _get_current_version()}

    @router.post("/api/asap/config/rollback/{version}")
    async def rollback_config(version: int, request: Request):
        """回滚到指定版本"""
        from .config import rollback_config, _load_unified
        result = rollback_config(version)
        if result.get("success"):
            _load_unified(request.app.state.config)
        return result
    ...
    def _get_current_version() -> int:
        """获取当前配置版本号"""
        import os as _os
        fp = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "config.toml")
        if not _os.path.exists(fp):
            return 0
        try:
            with open(fp, "rb") as f:
                import tomllib
                return tomllib.load(f).get("meta", {}).get("version", 0)
        except Exception:
            return 0
